...
int main(){
    ...
    while(scanf("%d", &situation) != EOF) {     
        for(int j = 0; j < situation; j++) {
            // Create 10000 buckets
			struct node *harsing[10000] = {NULL};
            for(int i=0; i<10000; i++){
            	harsing[i] = (node*)malloc(sizeof(node));
            	harsing[i]->next = NULL;
			}
			
			// Throw source[i] to the corresponding bucket by its highest five digits
			int amount;  // number of the nodes       
            scanf("%d", &amount);			 
            for(int i = 0; i < amount; i++) {
                struct node *new_node = (node *)malloc(sizeof(node));
                scanf("%d", &(new_node->value));
                // Insert to the list from the front position
                new_node->next = harsing[(new_node->value)/1000]->next; 
                harsing[(new_node->value)/1000]->next = new_node;
            }
            
            // Sort and output the linked buckets
            for(int i=0; i<10000; i++){
            	if(harsing[i] != NULL){
            		merge_sort(&(harsing[i]->next));
		            print_list(harsing[i]->next);
		            free_list(harsing[i]);
				}
			}
